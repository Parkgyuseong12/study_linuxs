...
file="words.txt" #변수 지정

read -p "big small? (y/n) : " choice  #read 명령어로 입력 받기

vi word_count.sh

if [ "$choice" = "y" ]; then   #if문 y 입력값 받았을 때 
        echo " big : on" #big : on 출력
        sort "$file" | uniq -c | sort -nr # 변수 file 값을 정렬 중복 삭제 빈도수 세고 정렬
        count=$(sort "$file" | uniq | wc -l) #변수 file 정렬 중복 삭제 라인 갯수 변수 설정
        echo "all count : $count" # 위의 출력된 숫자 출력
        topword=$(sort "$file" | uniq -c | sort -nr | head -1 | awk '{print $1}')

        if [ "$topword" -ge 3 ]; then
                echo "same : high same"
        elif [ "$topword" -eq 2 ]; then
                echo "same : middle same"
        else
                echo "same : low same"
        fi
elif [ "$choice" = "n" ]; then
        echo "small : off"
        tr 'A-Z' 'a-z' < "$file" | sort | uniq -c | sort -nr
else
        echo " warning enter the y or n"
        exit 1
fi

[parkgyuseong@localhost shell_practice]$ source word_count.sh
big small? (y/n) : y
 big : on
      2 banana
      2 Apple
