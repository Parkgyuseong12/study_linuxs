...
```
nano wordcount.sh       # 스크립트 창 입장
read -p  "Enter filename : " read1 # read 명령어 사용
word_count=$(wc -w < "$read1") # 변수 지정
echo "Word count in " "$read1 : $word_count"

[parkgyuseong@localhost env]$ bash wordcount.sh
Enter filename : file1.txt
Word count in  file1.txt : 12
...
```
[parkgyuseong@localhost env]$ nano count_keyword.sh
                
word="$1"
file="$2"

count=$(grep -i -o "$word" "$file" | wc -w)
echo "The word "$word" appeared "$count" times"

[parkgyuseong@localhost env]$ bash count_keyword.sh error logfile.txt
